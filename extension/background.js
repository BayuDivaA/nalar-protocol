...
async function handleSecurityCheck(transaction, chainId, origin) {
  ...
  if (!intent) {
    throw new Error(
      "Set your transaction intent in the Nalar extension popup first.",
    );
  }
  ...
  const response = await fetch(
    `${BACKEND_URL}/api/transactions/security-check`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `TxSentry returned ${response.status}: ${responseText}`,
    );
  }
...
function hexToDecimal(value) {
  ...
  if (value.startsWith("0x")) {
    try {
      ...
      throw new Error(
        "Transaction value is not a valid hexadecimal quantity.",
      );
    }
  }
  ...
  throw new Error(
    "Transaction value must be a hexadecimal or decimal quantity.",
  );
}
